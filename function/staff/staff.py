...
class StaffWindow:
    ...
    def add(self):
        ...
        if (re.findall(intPattern, self.name.get()) or re.findall(intPattern,
                                                                  self.last.get()) or self.name.get() == "" or self.last.get() == ""):
            messagebox.showwarning("Warning", "Invalid Name ! Should be a string !")
            return

        for d in cur:
            max_id = int(d['ID']) + 1
        data = {"ID": max_id, "Firstname": self.name.get(), "Lastname": self.last.get(), "Phone": self.phone.get(),
                "Email": self.email.get(), "Job": self.job.get()}
    ...
    def update(self):
        ...
        if (re.findall(intPattern, self.name.get()) or re.findall(intPattern,
            ...
            messagebox.showwarning("Warning", "Invalid Name ! Should be a string !")
            return
        new_data = {"Firstname": self.name.get(), "Lastname": self.last.get(), "Email": self.email.get(),
                    "Phone": self.phone.get(), "Job": self.job.get()}
